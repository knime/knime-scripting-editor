...
import { getInitialDataService, getScriptingService, getSettingsService, init } from "@/init";

// TODO this can maybe be simpler?
...
describe("init", () => {
  ...
  describe("display mode management", () => {
    it("sets initial display mode from dialog service", async () => {
      ...
      expect(dialogServiceInstance.getInitialDisplayMode).toHaveBeenCalledOnce();
    ...
    it("registers display mode change callback", async () => {
      ...
      expect(dialogServiceInstance.addOnDisplayModeChangeCallback).toHaveBeenCalledOnce();
      expect(typeof dialogServiceInstance.addOnDisplayModeChangeCallback.mock.calls[0][0]).toBe("function");
    });
    ...
    it("updates display mode when callback is triggered", async () => {
      ...
      const callback = dialogServiceInstance.addOnDisplayModeChangeCallback.mock.calls[0][0];
