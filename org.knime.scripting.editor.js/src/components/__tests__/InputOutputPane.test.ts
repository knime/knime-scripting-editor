...
describe("InputOutputPane", () => {
  ...
  it("renders input / output items", async () => {
    ...
    expect(inpOupPanes.length).toBe(
      numInputObjects + numOutputObjects + numFlowVariables,
    );
    expect(inpOupPanes.at(0)!.props().inputOutputItem).toEqual(
      DEFAULT_INITIAL_DATA.inputObjects[0],
    );
    expect(inpOupPanes.at(numInputObjects)!.props().inputOutputItem).toEqual(
      DEFAULT_INITIAL_DATA.flowVariables,
    );
    expect(
      inpOupPanes.at(numFlowVariables + numInputObjects)!.props()
        .inputOutputItem,
